...
# March 2023
# Descriptive trend visualisation (Figures 1 and 2)
...
#import
#d1 <- read.csv("Eurostat_cit_data_2023-03-02.csv", stringsAsFactors = FALSE)
...
fig1 <- d2 %>% 
  ...
  labs(title = "Figure 1. Over a hundred thousand British nationals acquired citizenship elsewhere in Europe in the 6 years since the Brexit referendum",
...
#Save as jpeg file
jpeg("Fig1_citaq_UK_total.jpeg", width=12, height=8, units="in", res=200)
fig1
dev.off()
...
fig2 <- d3 %>% 
  ...
  theme(legend.position = "none")+
  theme(plot.title = element_text(face = "bold"))+
  labs(title = "Figure 2. 60 percent of 100k post-Brexit referendum naturalisations by British in 3 countries: Germany, France, Sweden",
...
#Save as jpeg file
jpeg("Fig2_citaq_UK_iso_d.jpeg", width=12, height=8, units="in", res=500)
fig2
dev.off()
